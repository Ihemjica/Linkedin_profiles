...
class LinkedinProfiles():
    def __init__(self, company, email_suffix):
        self.base_url = "https://www.linkedin.com/checkpoint/lg/login-submit"
        ...
        self.username = 'ihemjica@gmail.com'#raw_input('linkedin username:')
        self.password = 'MasterJobber12'#raw_input('linkedin password:')
    ...
    def get_linkedin_profiles(self):
        #Preliminary request to get required cookie
        source = requests.get(self.base_url)
        
        #Cookie required for a succesful login
        bcookie = source.cookies['bcookie'].strip('"')[4:]
        cookies = {'bcookie' : source.cookies['bcookie'].strip('"')}
        data = {'session_key': self.username, 'session_password': self.password, 'loginCsrfParam': bcookie}
        #login request
        login = requests.post(self.base_url, cookies = cookies, data = data, allow_redirects = False)
        
        #Extracting cookies for touring the site
        JSESSIONID = login.cookies['JSESSIONID'].strip('"')
        LI_AT = login.cookies['li_at']
